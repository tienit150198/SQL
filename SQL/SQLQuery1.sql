...
create table DATA(ID int identity primary key not null,URL nvarchar(200) unique, source nvarchar(100), title nvarchar(200), type nvarchar(100), post_time date default getdate(), crawl_time date default getdate(), description nvarchar(max), content nvarchar(max), tags nvarchar(max), author nvarchar(100), link_image nvarchar(max));
go;
...
create proc DT_GetData_InDay_crawltime(@Day nvarchar(10)) as
begin
	select * from DATA where day(crawl_time) = @Day;
end
go;

create proc DT_GetData_InMonth_crawltime(@Month nvarchar(10)) as
begin
	select * from DATA where month(crawl_time) = @Month;
end
go;

create proc DT_GetData_InYear_crawltime(@Year nvarchar(10)) as
begin
...
create proc DT_Add(@URL nvarchar(200) , @source nvarchar(100), @title nvarchar(200), @type nvarchar(100), @post_time date, @description nvarchar(max), @content nvarchar(max), @tags nvarchar(max), @author nvarchar(100), @link_image nvarchar(max)) as
begin
	if exists (select * from DATA where @URL = URL)
		print 'URL not exists';
	else
		begin
			insert into DATA(URL, source, title, type, post_time, crawl_time, description, content, tags, author, link_image)
			values (@URL, @source, @title, @type, @post_time, getdate(), @description, @content, @tags, @author, @link_image)
		end
end
